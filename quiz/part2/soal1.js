...
function digitPerkalianMinimum(angka) {
  ...
  for (let i = 1; i <= akarKuadrat; i++) {
    if (angka % i === 0) {
      let pasangan = angka / i;
      let digitGabungan = i.toString() + pasangan.toString();
      hasil.push(digitGabungan);
    }
  }

  let digitMinimum = hasil[0].length;

  for (let stringGabungan of hasil) {
    let panjangSekarang = stringGabungan.length;
    if (panjangSekarang < digitMinimum) {
      digitMinimum = panjangSekarang;
    }
  }
  return digitMinimum;
}
