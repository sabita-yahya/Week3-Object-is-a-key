...
function countProfit(shoppers) {
  ...
  for (const pembeli of shoppers) {
    for (const barang of hasilAkhir) {
      if (
        pembeli.product === barang.product &&
        barang.leftOver >= pembeli.amount
      ) {
        barang.shoppers.push(pembeli.name);
        barang.leftOver -= pembeli.amount;

        barang.totalProfit += pembeli.amount * prices[barang.product];
      }
    }
  }
  return hasilAkhir;
